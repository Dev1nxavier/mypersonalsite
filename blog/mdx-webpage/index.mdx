...
import image from './brain_diagram_hero_forma.jpeg';
import {
  Card,
  CardActions,
  CardContent,
...
### _A note on frontmatter_:

frontmatter lets us access page medidata such as title, hero images, dates and more which can be accessed via GraphQL query. To query data, MDX files must be added to the site Node system using a plugin such as gatsby-source-filesystem. Visit [Gatsby Source Filesystem](/blog/howtoquery) for instructions.
...
MDX is a _writing_ language (as opposed to html which is a _publishing_ language). That means its most suited for basic formatting of plain-text. Once we have our basic content written, there are a few things we can do to improve the look of the page.
...
2.  Under the page medata write content that will be the body of your page.  
    _Not inspired to write that novel yet? Trying using [Lorem Ipsum](https://www.lipsum.com/) for filler._
...
3.  Import Components into your MDX file  
    ...
    _Using mui's card component is just an example. You are free to use whatever library or custom component you want!_
...
4.  Start populating your components  
    Just like with the page title, you can access medidata directly without a GraphQL query. Below is an example using Material-UI's card component   
            <Card sx={{width:345}}>
                <CardMedia component='img' height="140" image={props.pageContext.frontmatter.hero_image}/>
                <Typograpy variant="h5" component="div">
                    "Building a Gatsby webpage from MDX"
                </Typography>
                <Typography variant="body2">
                "MDX let's you focus on the blogging while keeping all your content consistent!"
                </Typography>
            </Card>   
        
# And there we go! Our final render. Now you can create one template page for all your blogs and publish easily! Happy blogging! #    
            

<Card sx={{ width: 345, justifySelf:'center' }} elevation={10}>
  <CardContent>
    <CardMedia
        component="img"
        height="140"
        image={image}
        alt="2D brain in neon lights"
      />
    <Typography gutterBottom variant="h5" component="div">Building a Gatsby webpage from MDX</Typography>
    <Typography variant="body2">
      MDX let's us focus on the blogging while keeping all your content
      consistent!
    </Typography>
  </CardContent>
</Card>
